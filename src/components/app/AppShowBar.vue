...
// Close button
const closeShowBar = () => {
  // set showBar false value
  showBarValue.value = false;
};
</script>

<template>
  <div class=" bg-[#5596f7]">
    <div class="max-w-[1080px] px-[14px] xl:px-0 mx-auto" v-if="showBarValue">
      <div class="flex py-2 items-center justify-between relative underline">
        <a  href="https://github.com/itsaslanov/stylesheets" target="_blank" class="flex items-center gap-1 text-[14px] md:text-[16px]">
          <span class="heart-animation">
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 512 384">
              <path fill="#CF649A"
          ...
          <p class="w-[80%] md:w-[100%]">
            Hey there. If you are a fan of Sass, then check out my Sass stylesheet from my github repository.
          </p>
        </a>


        <button title="Close" @click="closeShowBar"
          class="bg-[white] block text-[#333] rounded-full w-[25px] h-[25px] opacity-100 hover:opacity-70 absolute right-0">
          &#x2715
        </button>
      </div>
    </div>
